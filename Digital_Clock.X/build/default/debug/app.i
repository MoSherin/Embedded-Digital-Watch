...
# 34 "./ECU-Layer/LED/../../MCAL_Layer/GPIO/../mcal_std_types.h"
...
# 18 "./ECU-Layer/RELAY/ecu_relay.h"
typedef enum{
    RELAY_OFF,
    RELAY_ON
}relay_state_t;
...
# 18 "./ECU-Layer/MOTOR/ecu_motor.h"
typedef enum{
    MOTOR_OFF,
    MOTOR_ON
}motor_state_t;
...
# 16 "./MCAL_Layer/Interupt/hal_ext_interupt.h"
...
EXT_Interupt_t mode_button = {.interupt_pin.port = PORTB_INDEX, .interupt_pin.pin = PIN0, .interupt_pin.direction = INPUT, .interupt_pin.logic = LOW, .Edge = Rising_Edge, .EXT_InteruptHandler = IN0_APP_ISR, .source = INT0_PIN};
EXT_Interupt_t pause_button = {.interupt_pin.port = PORTB_INDEX, .interupt_pin.pin = PIN1, .interupt_pin.direction = INPUT, .interupt_pin.logic = LOW, .Edge = Rising_Edge, .EXT_InteruptHandler = IN1_APP_ISR, .source = INT1_PIN};
EXT_Interupt_t reset_button = {.interupt_pin.port = PORTB_INDEX, .interupt_pin.pin = PIN2, .interupt_pin.direction = INPUT, .interupt_pin.logic = LOW, .Edge = Rising_Edge, .EXT_InteruptHandler = IN2_APP_ISR, .source = INT2_PIN};


timer0_t timer = {.bit_mode = TIMER0_16BIT_MODE, .interrupt_priority = 0, .mode = TIMER0_CLOCK_MODE,
...
void diplay_time(uint8 hours, uint8 mins, uint8 secs){
    if(!(3 == mode && 3 == set_mode && 1 == delay_flag) || 1 == mode || 2 == mode)
    {
    ...
    if(!(3 == mode && 2 == set_mode && 1 == delay_flag) || 1 == mode || 2 == mode)
    {
    ...
    if(!(3 == mode && 1 == set_mode && 1 == delay_flag) || 1 == mode || 2 == mode)
    {
...
void IN0_APP_ISR(void){
    ...
    {
        stop_milli_seconds = 0; stop_seconds = 0; stop_minutes = 0;
    }
    else { }
}
...
void TMR0_APP_ISR(void){
    ...
    else if(2 == mode)
    {
        ...
        {
            stop_milli_seconds++;
            if(80 == stop_milli_seconds)
            {
                stop_milli_seconds = 0;
                stop_seconds++;
            }
            else if(60 == stop_seconds)
            {
                stop_seconds = 0;
                stop_minutes++;
            }
            else if(60 == stop_minutes)
            {
                stop_minutes = 0;
            }
            else { }
